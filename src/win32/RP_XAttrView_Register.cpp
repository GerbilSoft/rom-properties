 ...
 */
LONG RP_XAttrView::RegisterFileType_int(RegKey &hkey_Assoc)
{
	...
	LONG lResult = hkcr_PropSheet.write(nullptr, CLSID_RP_XAttrView_String);
 ...
 * @return ERROR_SUCCESS on success; Win32 error code on error.
 */
LONG RP_XAttrView::UnregisterFileType_int(RegKey &hkey_Assoc)
{
	...
	// Open the "XAttrView" property sheet handler key.
	RegKey hkcr_PropSheet(hkcr_PropSheetHandlers, psh_name, KEY_READ, false);
	if (!hkcr_PropSheet.isOpen()) {
		// ERROR_FILE_NOT_FOUND is acceptable here.
	...
	if (str_PropSheetCLSID == CLSID_RP_XAttrView_String) {
		// Default value matches.
		// Remove the subkey.
		hkcr_PropSheet.close();
		LONG lResult = hkcr_PropSheetHandlers.deleteSubKey(psh_name);
		if (lResult != ERROR_SUCCESS) {
			return lResult;
		}
	} else {
