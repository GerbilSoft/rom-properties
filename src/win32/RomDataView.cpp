 ...
 * RomDataView.cpp: RomData viewer control.                                *
 *                                                                         *
 ...
 */
void RomDataViewPrivate::loadImages(void)
{
 ...
 */
int RomDataViewPrivate::initString(_In_ HWND hWndTab,
 ...
 * @return Field height, in pixels.
 */
int RomDataViewPrivate::initBitfield(_In_ HWND hWndTab,
 ...
 * @return Field height, in pixels.
 */
int RomDataViewPrivate::initListData(_In_ HWND hWndTab,
 ...
 * @return Field height, in pixels.
 */
int RomDataViewPrivate::initDateTime(_In_ HWND hWndTab,
	_In_ POINT pt_start, _In_ SIZE size,
	_In_ const RomFields::Field &field, _In_ int fieldIdx)
{
 ...
 * @return Field height, in pixels.
 */
int RomDataViewPrivate::initAgeRatings(_In_ HWND hWndTab,
	_In_ POINT pt_start, _In_ SIZE size,
	_In_ const RomFields::Field &field, _In_ int fieldIdx)
{
 ...
 * @return Field height, in pixels.
 */
int RomDataViewPrivate::initDimensions(_In_ HWND hWndTab,
	_In_ POINT pt_start, _In_ SIZE size,
	_In_ const RomFields::Field &field, _In_ int fieldIdx)
{
 ...
 * @return Field height, in pixels.
 */
int RomDataViewPrivate::initStringMulti(_In_ HWND hWndTab,
	_In_ POINT pt_start, _In_ SIZE size,
	_In_ const RomFields::Field &field, _In_ int fieldIdx)
{
 ...
 */
inline BOOL RomDataViewPrivate::ListView_GetDispInfo(NMLVDISPINFO *plvdi)
{
 ...
 * @return TRUE if handled; FALSE if not.
 */
inline BOOL RomDataViewPrivate::ListView_ColumnClick(const NMLISTVIEW *plv)
{
 ...
 * @return TRUE if handled; FALSE if not.
 */
inline BOOL RomDataViewPrivate::Header_DividerDblClick(const NMHEADER *phd)
{
 ...
 * @return Return value.
 */
inline int RomDataViewPrivate::ListView_CustomDraw(NMLVCUSTOMDRAW *plvcd) const
{
 ...
 * @return Return value
 */
INT_PTR RomDataViewPrivate::WndProc_WM_NOTIFY(HWND hWnd, NMHDR *pHdr)
{
	INT_PTR ret = FALSE;
...
/**
 * WM_COMMAND handler for RomDataView
 * @param hWnd RomDataView
 * @param wParam
 * @param lParam
 * @return Return value.
...
/**
 * WM_PAINT handler for RomDataView
 * @param hWnd RomDataView
 * @return Return value.
 */
INT_PTR RomDataViewPrivate::WndProc_WM_PAINT(HWND hWnd)
{
...
//
INT_PTR CALLBACK RomDataViewPrivate::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	// Based on CppShellExtPropSheetHandler.
	// https://code.msdn.microsoft.com/windowsapps/CppShellExtPropSheetHandler-d93b49b7
	switch (uMsg) {
		case WM_NCCREATE: {
			// Create a private class object.
			// TODO: Filename?
			RomDataViewPrivate *const d = new RomDataViewPrivate(hWnd, nullptr);
		...
		case WM_COMMAND: {
			...
			d->WndProc_WM_COMMAND(hWnd, wParam, lParam);
			break;
		}
		...
		case WM_THEMECHANGED: {
			...
			// TODO: Get the top-most window?
			d->isDarkModeEnabled = VerifyDialogDarkMode(GetParent(hWnd));
		...
		case WM_ROMDATAVIEW_ANIMATION_CTRL:
			...
			if (wParam) {
				// Non-zero: Start animations.
				if (d->lblIcon) {
					d->lblIcon->startAnimTimer();
				}
				if (d->hBtnOptions) {
					ShowWindow(d->hBtnOptions, SW_SHOW);
				}
			} else {
				// Zero: Stop animations.
				if (d->lblIcon) {
					d->lblIcon->stopAnimTimer();
				}
				if (d->hBtnOptions) {
 ...
 * @param lParam
 */
INT_PTR CALLBACK RomDataViewPrivate::SubtabDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg) {
		case WM_DESTROY: {
			// Remove the TAB_PTR_PROP property from the page.
			// The TAB_PTR_PROP property stored the pointer to the
			// RomDataViewPrivate::tab object.
			RemoveProp(hDlg, RomDataViewPrivate::TAB_PTR_PROP);
			break;
		}
		...
		case WM_CTLCOLORSCROLLBAR: {
			...
			// to enforce Dark Mode schemes in Windows Explorer.
			// TODO: Handle color scheme changes?
			auto* const d = reinterpret_cast<RomDataViewPrivate*>(GetWindowLongPtr(hDlg, GWLP_USERDATA));
			if (d && d->isDarkModeEnabled) {
		...
		case WM_VSCROLL: {
			auto *const d = reinterpret_cast<RomDataViewPrivate*>(GetWindowLongPtr(hDlg, GWLP_USERDATA));
			auto *const tab = static_cast<RomDataViewPrivate::tab*>(GetProp(hDlg, TAB_PTR_PROP));
			if (!d || !tab) {
				// No RomDataViewPrivate or tab. Can't do anything...
				break;
			}
...
void RomDataViewRegister(void)
{
	...
	static const WNDCLASSEX wndClass = {
		...
		RomDataViewPrivate::WndProc,	// lpfnWndProc
		0,				// cbClsExtra
