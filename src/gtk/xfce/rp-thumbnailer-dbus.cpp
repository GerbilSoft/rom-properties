...
enum CheckFileMtimes {
	CHECK_FILE_MTIMES_NEEDS_UPDATE = 0,
	CHECK_FILE_MTIMES_UP_TO_DATE,
	CHECK_FILE_MTIMES_SOURCE_ERROR,
};

/**
 * Check file modification times.
 * @param source_file	[in] Source file being thumbnailed. (path, not uri)
 * @param output_file	[in] Thumbnail cache file. (path, not uri)
 * @return CheckFileMtime
 */
static CheckFileMtimes
check_file_mtimes(const gchar *source_file, const gchar *output_file)
{
	// If source_file isn't readable, this is an error.
	// if output_file isn't readable, this means an update is required.
	GFile *f_src = nullptr;
	GFile *f_out = nullptr;
	GFileInfo *fi_src = nullptr;
	GFileInfo *fi_out = nullptr;
	GError *error = nullptr;
	CheckFileMtimes ret;
	uint64_t src_sec, out_sec;

	f_src = g_file_new_for_path(source_file);
	if (!f_src) {
		ret = CHECK_FILE_MTIMES_SOURCE_ERROR;
		goto cleanup;
	}
	f_out = g_file_new_for_path(output_file);
	if (!f_out) {
		ret = CHECK_FILE_MTIMES_NEEDS_UPDATE;
		goto cleanup;
	}

	// Get the file information.
	fi_src = g_file_query_info(f_src,
	...
	fi_out = g_file_query_info(f_out,
		G_FILE_ATTRIBUTE_TIME_MODIFIED ","
		G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC,
		G_FILE_QUERY_INFO_NONE, nullptr, &error);
	if (error) {
	...
	// Make sure both files have mtime attributes.
	// (usec attribute is optional.)
	if (!g_file_info_has_attribute(fi_src, G_FILE_ATTRIBUTE_TIME_MODIFIED)) {
		ret = CHECK_FILE_MTIMES_SOURCE_ERROR;
		goto cleanup;
	}
	if (!g_file_info_has_attribute(fi_out, G_FILE_ATTRIBUTE_TIME_MODIFIED)) {
		ret = CHECK_FILE_MTIMES_NEEDS_UPDATE;
		goto cleanup;
	}

	// Get the attributes.
	src_sec = g_file_info_get_attribute_uint64(fi_src, G_FILE_ATTRIBUTE_TIME_MODIFIED);
	out_sec = g_file_info_get_attribute_uint64(fi_out, G_FILE_ATTRIBUTE_TIME_MODIFIED);
	if (src_sec > out_sec) {
		// Source is newer.
		ret = CHECK_FILE_MTIMES_NEEDS_UPDATE;
	} else if (src_sec == out_sec) {
		// Check usec.
		uint32_t src_usec = g_file_info_get_attribute_uint64(fi_src, G_FILE_ATTRIBUTE_TIME_MODIFIED);
		uint32_t out_usec = g_file_info_get_attribute_uint64(fi_out, G_FILE_ATTRIBUTE_TIME_MODIFIED);
		if (src_usec > out_usec) {
			// Source is newer.
			ret = CHECK_FILE_MTIMES_NEEDS_UPDATE;
		} else {
			// Thumbnail is up to date.
			ret = CHECK_FILE_MTIMES_UP_TO_DATE;
		}
	} else {
		// Thumbnail is up to date.
		ret = CHECK_FILE_MTIMES_UP_TO_DATE;
...
cleanup:
	...
	if (f_out) {
		g_object_unref(f_out);
	}
	if (fi_src) {
		g_object_unref(fi_src);
	}
	if (fi_out) {
		g_object_unref(fi_out);
	}
	return ret;
}
...
rp_thumbnailer_process(RpThumbnailer *thumbnailer)
{
	...
	// If the thumbnail already exists and its timestamp is
	...
	switch (ret) {
		...
		default:
			...
			break;
		case CHECK_FILE_MTIMES_UP_TO_DATE:
			// Thumbnail is up to date.
			g_debug("rom-properties thumbnail: %s -> %s [OK,NC]", filename, cache_filename.c_str());
			org_freedesktop_thumbnails_specialized_thumbnailer1_emit_ready(
				thumbnailer->skeleton, handle, req->uri.c_str());
			goto cleanup;
		case CHECK_FILE_MTIMES_SOURCE_ERROR:
			// An error occurred while reading the source file mtime.
			org_freedesktop_thumbnails_specialized_thumbnailer1_emit_error(
				thumbnailer->skeleton, handle, req->uri.c_str(),
