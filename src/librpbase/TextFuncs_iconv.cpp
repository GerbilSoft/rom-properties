 ...
 * @return UTF-8 string
 */
u8string cpN_to_utf8(unsigned int cp, const char *str, int len, unsigned int flags)
{
	if (cp & CP_RP_BASE) {
		// RP-custom code page.
	...
	char8_t *mbs = reinterpret_cast<char8_t*>(rp_iconv(str, len*sizeof(*str), cp_name, "UTF-8", ignoreErr));
	if (!mbs /*&& (flags & TEXTCONV_FLAG_CP1252_FALLBACK)*/) {
		// Try cp1252 fallback.
		// NOTE: Sometimes cp1252 fails, even with ignore set.
		if (cp != 1252) {
			mbs = reinterpret_cast<char8_t*>(rp_iconv(str, len*sizeof(*str), "CP1252", "UTF-8", true));
		}
		if (!mbs) {
			// Try Latin-1 fallback.
			if (cp != CP_LATIN1) {
				mbs = reinterpret_cast<char8_t*>(rp_iconv(str, len*sizeof(*str), "LATIN1", "UTF-8", true));
			}
		}
	}
	...
	if (mbs) {
		...
		if (cp == CP_SJIS) {
			...
			const auto s_ret_end = s_ret.end();
			for (auto p = s_ret.begin(); p != s_ret_end; ++p) {
...
u16string cpN_to_utf16(unsigned int cp, const char *str, int len, unsigned int flags)
{
	...
	if (wcs) {
		...
		if (cp == CP_SJIS) {
			...
			// map it to U+FF5E.
			std::replace(ws_ret.begin(), ws_ret.end(), (char16_t)0x301C, (char16_t)0xFF5E);
		}
#endif /* HAVE_ICONV_LIBICONV */
	}
	return ws_ret;
 ...
 * @return 8-bit text
 */
string utf8_to_cpN(unsigned int cp, const char8_t *str, int len)
{
	len = check_NULL_terminator(str, len);
	...
	string s_ret;
	char *const mbs = rp_iconv(
		reinterpret_cast<const char*>(str), len*sizeof(*str), "UTF-8", cp_name, true);
	if (mbs) {
		s_ret.assign(mbs);
		free(mbs);
	}
...
string utf16_to_cpN(unsigned int cp, const char16_t *wcs, int len)
{
	...
	string s_ret;
	char *const mbs = rp_iconv(
		reinterpret_cast<const char*>(wcs), len*sizeof(*wcs), RP_ICONV_UTF16_ENCODING, cp_name, ignoreErr);
	if (mbs) {
		s_ret.assign(mbs);
		free(mbs);
	}
 ...
 * @return UTF-8 string
 */
u8string utf16le_to_utf8(const char16_t *wcs, int len)
{
	len = check_NULL_terminator(wcs, len);

	// Attempt to convert the text from UTF-16LE to UTF-8.
	u8string s_ret;
	char8_t *const mbs = reinterpret_cast<char8_t*>(rp_iconv(
		reinterpret_cast<const char*>(wcs), len*sizeof(*wcs), "UTF-16LE", "UTF-8"));
	if (mbs) {
		s_ret.assign(mbs);
		free(mbs);
	}
 ...
 * @return UTF-8 string
 */
u8string utf16be_to_utf8(const char16_t *wcs, int len)
{
	len = check_NULL_terminator(wcs, len);
	...
	u8string s_ret;
	char8_t *const mbs = reinterpret_cast<char8_t*>(rp_iconv(
		reinterpret_cast<const char*>(wcs), len*sizeof(*wcs), "UTF-16BE", "UTF-8"));
	if (mbs) {
		s_ret.assign(mbs);
		free(mbs);
	}
