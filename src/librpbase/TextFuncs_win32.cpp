...
u8string cpN_to_utf8(unsigned int cp, const char *str, int len, unsigned int flags)
{
	...
	if (wcs && cchWcs > 0) {
		...
		int cbMbs;
		char8_t *const mbs = reinterpret_cast<char8_t*>(
			W32U_UTF16_to_mbs(wcs, cchWcs, CP_UTF8, &cbMbs));
		if (mbs && cbMbs > 0) {
...
string utf8_to_cpN(unsigned int cp, const char8_t *str, int len)
{
	...
	string s_ret;
	int cchWcs;
	char16_t *wcs = W32U_mbs_to_UTF16(reinterpret_cast<const char*>(str), len, CP_UTF8, &cchWcs, 0);
	if (wcs && cchWcs > 0) {
		// Convert from UTF-16 to `cp`.
		int cbMbs;
		char *const mbs = W32U_UTF16_to_mbs(wcs, cchWcs, cp, &cbMbs);
		if (mbs && cbMbs > 0) {
			// Remove the NULL terminator if present.
			if (mbs[cbMbs-1] == 0) {
				cbMbs--;
 ...
 */
template<typename T>
static std::basic_string<T> T_utf16_to_cpN(unsigned int cp, const char16_t *wcs, int len)
{
	len = check_NULL_terminator(wcs, len);

	// Convert from UTF-16 to `cp`.
	std::basic_string<T> s_ret;
	int cbMbs;
	char *const mbs = W32U_UTF16_to_mbs(wcs, len, cp, &cbMbs);
	if (mbs && cbMbs > 0) {
		// Remove the NULL terminator if present.
		if (mbs[cbMbs-1] == 0) {
			cbMbs--;
		}
		s_ret.assign(reinterpret_cast<const T*>(mbs), cbMbs);
	}
	free(mbs);
	return s_ret;
}
...
{
	return T_utf16_to_cpN<char>(cp, wcs, len);
}
...
{
	// Wrapper around T_utf16_to_cpN().
	return T_utf16_to_cpN<char8_t>(CP_UTF8, wcs, len);
}
...
u8string utf16be_to_utf8(const char16_t *wcs, int len)
{
	...
	return T_utf16_to_cpN<char8_t>(CP_UTF8, bwcs.data(), static_cast<int>(bwcs.size()));
}
