...
/* TextureInfo */
const char8_t *const KhronosKTX2Private::exts[] = {
...
void KhronosKTX2Private::loadKeyValueData(void)
{
	...
	// Load the data.
	unique_ptr<char8_t[]> buf(new char8_t[ktx2Header.kvdByteLength]);
	...
	const char8_t *const p_end = p + ktx2Header.kvdByteLength;
 ...
 */
const char8_t *KhronosKTX2::textureFormatName(void) const
{
	RP_D(const KhronosKTX2);
	if (!d->isValid)
		return nullptr;

	return U8("Khronos KTX2");
}
