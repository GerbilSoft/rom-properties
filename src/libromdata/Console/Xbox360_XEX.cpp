 ...
 * @return Publisher
 */
u8string Xbox360_XEX_Private::getPublisher(void) const
{
	if (!isExecutionIDLoaded) {
 ...
 */
const char8_t *Xbox360_XEX::systemName(unsigned int type) const
{
...
int Xbox360_XEX::loadFieldData(void)
{
	...
	vector<u8string> *const v_module_flags = RomFields::strArrayToVector_i18n(
	...
	if (d->isExecutionIDLoaded) {
		...
		// Publisher
		const u8string publisher = d->getPublisher();
		if (!publisher.empty()) {
