...
int N64::loadFieldData(void)
{
	...
	const char8_t *const os_version_title = C_("N64", "OS Version");
	    ...
	    ISALPHA(romHeader->os_version[3]))
	{
		...
		char8_t buf[32];
		snprintf(reinterpret_cast<char*>(buf), sizeof(buf), "OS%u.%u%c",
	...
	const char8_t *const clockrate_title = C_("N64", "Clock Rate");
	...
	// CRCs
	{
		// FIXME: U8STRFIX - snprintf()
		char buf[32];
		snprintf(reinterpret_cast<char*>(buf), sizeof(buf), "0x%08X 0x%08X",
