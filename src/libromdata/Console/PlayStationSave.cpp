...
class PlayStationSavePrivate : public RomDataPrivate
{
	...
	public:
		...
		enum class SaveType {
			...
			Raw	= 1,	// Raw blocks without header information
			Block	= 2,	// Prefixed by header of the first block (*.mcs, *.ps1)
			PS54	= 3,	// Prefixed by 54-byte header (*.mcb, *.mcx, *.pda, *.psx)
	...
	: super(new PlayStationSavePrivate(this, file))
{
	...
	switch (d->saveType) {
		...
		case PlayStationSavePrivate::SaveType::Block:
			...
			break;
		case PlayStationSavePrivate::SaveType::PS54:
...
int PlayStationSave::isRomSupported_static(const DetectInfo *info)
{
	    ...
	    info->header.size < sizeof(PS1_SC_Struct))
	{
		...
		return (int)PlayStationSavePrivate::SaveType::Unknown;
	}
	...
	if (info->header.size >= sizeof(PS1_PSV_Header) + sizeof(PS1_SC_Struct)) {
		...
		if (scHeader->magic == cpu_to_be16(PS1_SC_MAGIC)) {
			...
			if (psvHeader->magic == cpu_to_be64(PS1_PSV_MAGIC)) {
				...
				return (int)PlayStationSavePrivate::SaveType::PSV;
			}
	...
	if (info->header.size >= sizeof(PS1_Block_Entry) + sizeof(PS1_SC_Struct)) {
		...
		if (scHeader->magic == cpu_to_be16(PS1_SC_MAGIC)) {
			...
			return (int)PlayStationSavePrivate::SaveType::Block;
		}
	}
	...
	if (info->header.size >= sizeof(PS1_54_Header) + sizeof(PS1_SC_Struct)) {
		...
		if (scHeader->magic == cpu_to_be16(PS1_SC_MAGIC)) {
			...
			return (int)PlayStationSavePrivate::SaveType::PS54;
		}
	}
	...
	if (scHeader->magic == cpu_to_be16(PS1_SC_MAGIC)) {
		...
		return (int)PlayStationSavePrivate::SaveType::Raw;
	}
...
int PlayStationSave::loadFieldData(void)
{
	...
	switch (d->saveType) {
		...
		case PlayStationSavePrivate::SaveType::PS54:
			...
			break;
		default:
			// No internal filename in this format.
			filename = nullptr;
			break;
	}
