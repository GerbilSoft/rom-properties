...
int GameCubeSave::isRomSupported_static(const DetectInfo *info)
{
	    ...
	    info->header.size < 1024)
	{
		...
		return (int)GameCubeSavePrivate::SaveType::Unknown;
	}
	...
	if (!memcmp(info->header.pData, gcs_magic, sizeof(gcs_magic))) {
		...
		if (data_size % 8192 == 0) {
			...
			{
				// This is a GCS file.
				return (int)GameCubeSavePrivate::SaveType::GCS;
			}
		}
	}
	...
	if (!memcmp(info->header.pData, sav_magic, sizeof(sav_magic)-1)) {
		...
		if (data_size % 8192 == 0) {
			...
			{
				// This is a GCS file.
				return (int)GameCubeSavePrivate::SaveType::SAV;
			}
		}
	}
	...
	if (data_size % 8192 == 0) {
		...
		{
			// This is a GCI file.
			return (int)GameCubeSavePrivate::SaveType::GCI;
		}
	}
