...
typedef enum {
	...
	RPCT_ERROR_DIRECTORY_THUMBNAILING_DISABLED	= 11,	// Directory thumbnailing is disabled. (v2+)
...
{
public:
	using RefImgClass = std::conditional<std::is_pointer<ImgClass>::value, ImgClass, ImgClass&>;
	using ConstRefImgClass = std::conditional<std::is_pointer<ImgClass>::value, ImgClass, const ImgClass&>;
