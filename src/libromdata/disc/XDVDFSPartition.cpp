	...
	, partition_size(partition_size)
{
	...
	size_t size = q->m_file->seekAndRead(
...
const rp::uvector<uint8_t> *XDVDFSPartitionPrivate::getDirectory(const char *path)
{
	...
	size_t size = q->m_file->seekAndRead(dir_addr, dirTable.data(), dirTable.size());
...
IRpFilePtr XDVDFSPartition::open(const char *filename)
{
	...
	return std::make_shared<PartitionFile>(this->shared_from_this(), file_addr, file_size);
}
