	...
	*langID = static_cast<unsigned int>(strtoul(str_langID.c_str(), &endptr, 16));
	...
	while ((tblPos + static_cast<int>(sizeof_fields)) < strTblData_len) {
		...
		if (value_len <= 0) {
			...
			// String bounds check
			assert(tblPos + ((value_len + 1) * static_cast<int>(sizeof(char16_t))) <= strTblData_len);
			if (tblPos + ((value_len + 1) * static_cast<int>(sizeof(char16_t))) > strTblData_len) {
				// Out of bounds.
				return -EIO;
			}
			value = reinterpret_cast<const char16_t*>(&strTblData[tblPos]);
			if (value[value_len] != cpu_to_le16(0)) {
				// Not NULL-terminated.
				return -EIO;
			}
		}
