...
class NintendoDSPrivate : public RomDataPrivate
{
	...
	public:
		...
		enum class RomType {
			...
			DSi_Only	= 3,	// Nintendo DSi-only ROM.
		};
		RomType romType;
...
void NintendoDS::init(void)
{
	...
	d->mimeType = (d->romType == NintendoDSPrivate::RomType::DSi_Only)
...
int NintendoDS::isRomSupported_static(const DetectInfo *info)
{
	    ...
	    info->header.size < sizeof(NDS_RomHeader))
	{
		...
		return (int)NintendoDSPrivate::RomType::Unknown;
	}
	...
	if (!memcmp(romHeader->nintendo_logo, nintendo_gba_logo, sizeof(nintendo_gba_logo)) &&
		...
		static const uint8_t nds_romType[] = {
			(uint8_t)NintendoDSPrivate::RomType::NDS,		// 0x00 == Nintendo DS
			(uint8_t)NintendoDSPrivate::RomType::NDS,		// 0x01 == invalid (assuming DS)
			(uint8_t)NintendoDSPrivate::RomType::DSi_Enhanced,	// 0x02 == DSi-enhanced
			(uint8_t)NintendoDSPrivate::RomType::DSi_Only,		// 0x03 == DSi-only
		};
		...
		return (int)NintendoDSPrivate::RomType::NDS_Slot2;
	}
...
const char *NintendoDS::systemName(unsigned int type) const
{
	...
	if (d->romType == NintendoDSPrivate::RomType::DSi_Only) {
		// DSi-exclusive game.
		idx |= (1U << 2);
...
int NintendoDS::loadFieldData(void)
{
		...
		romHeader->dsi.filetype != DSi_FTYPE_CARTRIDGE))
	{
		...
		if (d->romType == NintendoDSPrivate::RomType::NDS_Slot2) {
