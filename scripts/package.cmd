...
IF "%MSVC_ARM32_DIR%" == "" (
	...
	SET "CMAKE_ARM32_TOOLSET=%CMAKE_TOOLSET%"
)
...
ECHO - i386:    MSVC %MSVC32_YEAR% (%MSVC32_VERSION%) [WinXP: MSVC %MSVC_XP_YEAR% (%MSVC_XP_VERSION%)]
ECHO - amd64:   MSVC %MSVC64_YEAR% (%MSVC64_VERSION%) [WinXP: MSVC %MSVC_XP_YEAR% (%MSVC_XP_VERSION%)]
...
:: Compile the i386 version for Windows XP.
ECHO.
ECHO Compiling i386 (32-bit, Windows XP) rom-properties.dll...
MKDIR build.i386_xp
@IF ERRORLEVEL 1 EXIT /B %ERRORLEVEL%
PUSHD build.i386_xp
@IF ERRORLEVEL 1 EXIT /B %ERRORLEVEL%
...
MKDIR ..\pdb
MKDIR ..\pdb\i386_xp
FOR %%A IN (%PDB_FILES%) DO (COPY /Y "bin\Release\%%A" ..\pdb\i386_xp)
@IF ERRORLEVEL 1 EXIT /B %ERRORLEVEL%
POPD
...
:: Compile the amd64 version for Windows XP.
ECHO.
ECHO Compiling amd64 (64-bit, Windows XP) rom-properties.dll...
MKDIR build.amd64_xp
@IF ERRORLEVEL 1 EXIT /B %ERRORLEVEL%
...
@IF ERRORLEVEL 1 EXIT /B %ERRORLEVEL%
MKDIR ..\pdb\amd64_xp
FOR %%A IN (%PDB_FILES%) DO (COPY /Y "bin\Release\%%A" ..\pdb\amd64_xp)
@IF ERRORLEVEL 1 EXIT /B %ERRORLEVEL%
POPD
